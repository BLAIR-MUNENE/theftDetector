...
export default function CamerasPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 sm:space-y-8">
      <header>
        <h1 className="font-headline text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Cameras</h1>
      ...
      {loading ? (
        ...
        <>
          ...
          {cams.length > 0 && <ul className="space-y-2">{cams.map((c) => { const online = c.status === "open" || c.status === "running" || c.status === "active"; return <li key={c.id} className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] px-4 py-3.5 backdrop-blur-xl transition hover:bg-white/[0.05]"><div className="flex min-w-0 items-center gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/[0.06] ring-1 ring-white/[0.08]"><Camera className="h-4 w-4 text-muted" /></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-medium text-foreground">{c.name}</p><span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide" style={{ background: online ? "rgba(0,255,190,0.12)" : "rgba(110,120,145,0.15)", color: online ? "rgb(0,255,190)" : "rgb(110,120,145)", border: online ? "1px solid rgba(0,255,190,0.25)" : "1px solid rgba(110,120,145,0.2)" }}>{online ? <Wifi className="h-2.5 w-2.5" /> : <WifiOff className="h-2.5 w-2.5" />}{c.status}</span></div><p className="mt-0.5 break-all font-mono text-xs text-muted">{c.source}</p></div></div><div className="flex items-center gap-3"><label className="flex items-center gap-2 text-xs text-muted"><input type="checkbox" checked={!!startupSelected[c.id]} onChange={(e) => setStartupSelected((prev) => ({ ...prev, [c.id]: e.target.checked }))} className="h-4 w-4 rounded border-white/20 accent-[rgb(var(--accent-orange))]" />Startup</label><button type="button" onClick={() => remove(c.id)} className="rounded-xl p-2 text-muted transition hover:bg-red-950/40 hover:text-red-400" aria-label="Remove camera"><Trash2 className="h-4 w-4" /></button></div></li>; })}</ul>}
        </>
      )}
      <section className="space-y-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 backdrop-blur-xl">
