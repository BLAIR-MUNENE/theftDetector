  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between">
        <div>
          <h1 className="font-headline text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Alert history</h1>
          ...
          {rows.map((row) => (
            <li key={row.id} className="group flex flex-col gap-4 overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4 backdrop-blur-xl transition hover:bg-white/[0.05] hover:border-white/[0.12] sm:flex-row sm:items-start">
              <div className="relative shrink-0">
                ...
                <img src={alertImageUrl(row.image_path)} alt="" className="h-24 w-full rounded-xl border border-white/[0.08] object-cover sm:h-28 sm:w-40" />
